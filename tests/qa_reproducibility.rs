...
fn h0_1_different_seeds_produce_different_outputs() {
    ...
    for seed in seeds {
        ...
        engine.state_mut().add_body(1.0, Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0));
        
        // Use RNG to perturb state
        let perturbation = engine.rng_mut().gen_f64(); 
        engine.state_mut().add_body(1.0, Vec3::new(perturbation, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0));
...
fn h0_2_same_seed_produces_identical_outputs() {
    ...
    for i in 0..100 {
        ...
        // Run for some steps
        engine.run_for(simular::engine::SimTime::from_secs(0.1)).unwrap();
        ...
        if i == 0 {
            ...
            assert_eq!(state_str, first_output, "Run {} produced different output", i);
        }
    }
}
...
fn h0_6_checkpoint_restore_continuity() {
    ...
    engine1.state_mut().add_body(1.0, Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0));
    engine1.run_for(simular::engine::SimTime::from_secs(0.1)).unwrap();
    let final_state1 = serde_json::to_string(engine1.state()).unwrap();

    // Run 2: Interrupted
    let mut engine2 = SimEngine::new(config).unwrap();
    engine2.state_mut().add_body(1.0, Vec3::new(1.0, 0.0, 0.0), Vec3::new(0.0, 1.0, 0.0));
    
    // Run halfway
    engine2.run_for(simular::engine::SimTime::from_secs(0.05)).unwrap();
    ...
    *engine2.state_mut() = restored_state;
    ...
    assert_eq!(final_state1, final_state2, "Checkpoint restore produced different state");
}
...
fn h0_9_time_discretization_convergence() {
    ...
    engine1.state_mut().add_body(1.0, Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 0.0, 0.0));
    engine1.run_for(simular::engine::SimTime::from_secs(1.0)).unwrap();
    ...
    engine2.state_mut().add_body(1.0, Vec3::new(0.0, 0.0, 0.0), Vec3::new(1.0, 0.0, 0.0));
    engine2.run_for(simular::engine::SimTime::from_secs(1.0)).unwrap();
